...
```{r}
# obtaining  estimates if there was no censoring
glm.obj.q3b <- glm(wt82_71~
                 as.factor(qsmk) + 
                 cluster(seqn), 
               data = df.original, 
               weights = sw.c)
df.original$wtloss.p<-predict(glm.obj.q3b, df.original, type="response")
...
df.untreat$interv <- 0
df.untreat$qsmk<- 0
df.untreat$wt82_71 <- rep(NA, nrow(df.untreat))
df.untreat$wtloss.p <- rep(NA, nrow(df.untreat))

#create table 3 for untreated and label outcome as NA
df.treat <- df.original
df.treat$interv <- 1
df.treat$qsmk<- 1
df.treat$wtloss.p <- rep(NA, nrow(df.treat))

# create a dataset with 3 copies of each subject
df.q3<- as.data.frame(rbind(df.original, df.untreat, df.treat))

# estimate parametric model  
# HERE I SHOULD HAVE INCLUDED THE COVARIATES FOR THE CENSORED DATA
glm.fit.q3<-glm(wtloss.p~
                  as.factor(qsmk)+
                  as.factor(sex) + 
                  as.factor(race) + 
                  as.factor(exercise) + 
